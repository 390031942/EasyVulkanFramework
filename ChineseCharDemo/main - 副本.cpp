...
int main(int argc, char* argv[])
{
	...
	fontManager->Init();
	auto font = fontManager->LoadFont("test.ttf", 0, 48,false,false,true);;

	int cw_, ct_, bx_, by_, av_;
	float crd[4];

	unsigned char* bytes = NULL;
	bool result = fontManager->GetFontInfo(font, 'a', cw_, ct_, bx_, by_, av_, bytes, crd);
	...
	result = fontManager->GetFontInfo(font, L'°¡', cw_, ct_, bx_, by_, av_, bytes, crd);
...
EasyVulkanFontManager::VkFont::VkFont() :m_face(NULL),
m_rgba(NULL),
m_antialias(false),
m_bold(false),
m_tall(0)
...
EasyVulkanFontManager::VkFont::~VkFont()
{

}

bool EasyVulkanFontManager::VkFont::Create(FT_Library library, const char* filename, FT_Long face_index, int tall, bool bold, bool italic, bool antialias)
{
	FT_Error err;
...
void EasyVulkanFontManager::VkFont::Release()
{
	FT_Error err;
...
bool EasyVulkanFontManager::VkFont::GetCharInfo(int code, glyphMetrics* metrics, unsigned char *&bytes, float coords[])
{
	glyphMetrics gm;
...
int EasyVulkanFontManager::VkFont::GetFontTall()
{
	return m_tall;
}

bool EasyVulkanFontManager::VkFont::LoadChar(int code, glyphMetrics* metrics)
{
	FT_Error err;
...
void EasyVulkanFontManager::VkFont::VkFontChar::setInfo(glyphMetrics* metrics)
{
	memcpy(&m_metrics, metrics, sizeof(glyphMetrics));
}

void EasyVulkanFontManager::VkFont::VkFontChar::getInfo(glyphMetrics* metrics, unsigned char* &bytes, float coords[])
{
...
EasyVulkanFontManager::VkFont::VkFontPage::VkFontPage()
{
	m_wide = m_tall = 256;
	m_posx = m_posy = 0;
...
bool EasyVulkanFontManager::VkFont::VkFontPage::append(int wide, int tall, unsigned char* rgba, float coords[])
{
...
unsigned char* EasyVulkanFontManager::VkFont::VkFontPage::GetBytes()
{
	return m_data;
}
