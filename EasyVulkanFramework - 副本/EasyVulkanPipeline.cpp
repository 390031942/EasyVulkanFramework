...
void EasyVulkanPipeline::CreateUniformBuffer()
{
	/*片元着色*/
...
void EasyVulkanPipeline::DestoryUniformBuffer()
{
...
void EasyVulkanPipeline::CreatePipelineLayout()
{
	//由2变成3
...
void EasyVulkanPipeline::DestoryPipelineLayout()
{
...
void EasyVulkanPipeline::InitDescriptorSet()
{
	//由2变3
...
void EasyVulkanPipeline::CreateShader()
{
...
void EasyVulkanPipeline::InitVertexAttributeInfo()
{
...
void EasyVulkanPipeline::CreatePipeline(float screenWidth,float screenHeight)
{
...
void EasyVulkanPipeline::DestroyPipeline()
{
	//销毁管线
...
EasyVulkanPipeline::EasyVulkanPipeline(QSize ScreenSize,VkDevice device, VkRenderPass renderPass, VkPhysicalDeviceMemoryProperties memoryroperties)
{
	this->vkDevice = device;
	this->vkRenderPass = renderPass;
	this->vkMemoryProperties = memoryroperties;

	CreateUniformBuffer();//创建一致变量缓冲
	CreatePipelineLayout();//创建管线布局
	InitDescriptorSet();//初始化描述集
	EasyVulkanShaderCompiler::Init(device);
	CreateShader();//创建着色器
	InitVertexAttributeInfo();//初始化顶点属性信息
	CreatePipeline(ScreenSize.width(),ScreenSize.height());//创建管线
}

EasyVulkanPipeline::~EasyVulkanPipeline()
{
