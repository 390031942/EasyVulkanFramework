...
public:
	...
	void CreatePipeline()
	{
		Pipeline = new EasyVulkanPipeline(QSize(screenWidth, screenHeight), vkDevice, vkRenderPass, vkMemoryProperties);
	}
	void CreateFence()
	{
	...
	void InitDrawableResources()
	{
		...
		Drawable::SetCmdBuffer(vkCmdBuffer);
		Drawable::SetDevice(vkDevice);
		Drawable::SetPhysicalDevice(vkGpus[0]);

		Label::vkCmdBuffer = vkCmdBuffer;
		Label::vkPipelineLayout = Pipeline->vkPipelineLayout;



		/*m_triangle = new Triangle;
		m_triangle->SetColor4f(0.5, 0.5, 0.5,1);
		m_triangle->SetVertex2f(0, 0);

		m_triangle->SetColor4f(0.5, 1, 0.5,1);
		m_triangle->SetVertex2f(-100, 0);

		m_triangle->SetColor4f(1, 0.5, 0.5,1);
		m_triangle->SetVertex2f(0, 100);

		m_triangle->CreateVertexData();*/
