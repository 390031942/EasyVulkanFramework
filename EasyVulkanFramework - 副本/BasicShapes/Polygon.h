...
class PolygonShape :public Drawable
{
public:
	PolygonShape(int vertexCount,bool Indexed=false){
...
private:
	...
	VkMemoryRequirements* m_pMemoryReq;
	VkMemoryAllocateInfo* m_pMemoryAllocateInfo;
	VkDeviceSize          m_BufferByteCount;
	vector<uint16_t>      m_Indices;
	VkDeviceSize          m_VertexSize;
	VkDeviceSize          m_IndexSize;
	bool                  m_Indexed;
public:
	void  SetVertexIndex(vector<uint16_t> Indices)
	{
		if (!m_Indexed)
			return;
		m_Indices = Indices;
	...
	void  CreateVertexData()
	{
		...
		m_pBufferInfo->usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | (m_Indexed ? VK_BUFFER_USAGE_INDEX_BUFFER_BIT : 0);
